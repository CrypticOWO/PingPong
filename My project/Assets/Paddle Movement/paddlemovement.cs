...
public class paddlemovement: MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.UpArrow) && transform.position.y < yBorder)    //when O is pressed
        {
            transform.position = new Vector2(transform.position.x, transform.position.y + speed);   //move up
        }

        if (Input.GetKey(KeyCode.DownArrow) && transform.position.y > -yBorder)    //when L is pressed
        {
